...
interface WeChatOpenGuideProps {
  onClose?: () => void;
  // 添加一个控制是否显示的属性，默认不自动显示
  autoShow?: boolean;
}

export default function WeChatOpenGuide({ onClose, autoShow = false }: WeChatOpenGuideProps) {
  ...
  useEffect(() => {
    ...
    // 如果 autoShow 为 true，则检测并显示
    // 默认 autoShow 为 false，不会自动弹出
    if (autoShow) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('show-wechat-guide', handleShowGuide);
    return () => window.removeEventListener('show-wechat-guide', handleShowGuide);
  }, []);
